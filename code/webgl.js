...
function glInit() {
  ...
  glShader = glCreateProgram(
    "#version 300 es\n" + // specify GLSL ES version
      ...
      (glSpecular
        ? specularLightingCode // specular lighting test
...
function glPreRender(canvasSize) {
  ...
  glContext.uniformMatrix4fv(
    glUniform("m"),
    0,
    combinedMatrix.toFloat32Array()
  );
}

function glRender(transform = new DOMMatrix()) {
  // set up the lights and fog
  const initUniform4f = (name, x, y, z) =>
    glContext.uniform4f(glUniform(name), x, y, z, 0);
  ...
  const vertexData = glVertexData.subarray(
    0,
    glBatchCount * gl_INDICIES_PER_VERT
  );
...
function glCreateProjectionMatrix(fov = 0.5, near = 1, far = 1e4) {
...
function glPushVerts(points, normals, color, uvs) {
  ...
  if (!(count < gl_MAX_BATCH - glBatchCount)) glRender();
...
function glPushVertsCapped(points, normals, color, uvs) {
  ...
  const count = points.length;
  if (!(count + 2 < gl_MAX_BATCH - glBatchCount)) glRender();
...
function glPushColoredVerts(points, colors) {
  ...
  for (let i = count; i--; ) glPushVert(points[i], na, na, colors[i]);
