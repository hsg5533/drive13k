...
const levelLerpRange = 0.1;
const levelGoal = 10;
const playerStartZ = 2e3;
const turnWorldScale = 2e4;
const testStartZ = testLevel
  ? testLevel * checkpointDistance - 1e3
  : quickStart && !testLevelInfo
  ? 5e3
  : 0;
...
function gameInit() {
  ...
  if (quickStart || testLevel) titleScreenMode = 0;
  ...
  if (!clampAspectRatios) document.body.style.margin = "0px";
...
function gameStart() {
  time =
    ...
    playerLevel =
    playerWin =
    playerNewDistanceRecord =
    playerNewRecord =
    freeRide =
    checkpointSoundCount =
      0;
  startCountdown = quickStart || testLevel ? 0 : 4;
  worldHeading = titleScreenMode ? rand(7) : 0.8;
  ...
  vehicles.push(
    (playerVehicle = new PlayerVehicle(
      testStartZ ? testStartZ : playerStartZ,
      hsl(0, 0.8, 0.5)
    ))
  );
...
function gameUpdateInternal() {
  if (titleScreenMode) {
    ...
    if (
      mouseWasPressed(0) ||
      keyWasPressed("Space") ||
      (isUsingGamepad && (gamepadWasPressed(0) || gamepadWasPressed(9)))
    ) {
      titleScreenMode = 0;
      gameStart();
    }
    ...
    if (
      (gameOverTimer.get() > 1 &&
        (mouseWasPressed(0) ||
          (isUsingGamepad &&
            (gamepadWasPressed(0) || gamepadWasPressed(9))))) ||
      gameOverTimer.get() > 9
    ) {
    ...
    if (!startCountdown && !freeRide && !gameOverTimer.isSet()) {
      ...
      if (
        ...
        playerDistance > minRecordDistance
      ) {
        // new distance record
        sound_win.play(1, 2);
...
function gameUpdate(frameTimeMS = 0) {
  if (!clampAspectRatios)
    ...
    if (canvasFixedSize) {
      ...
      const correctedWidth =
        innerAspect > maxAspect
          ? innerHeight * maxAspect
          : innerAspect < minAspect
          ? innerHeight * minAspect
          : innerWidth;
      if (pixelate) {
        ...
        mainCanvasSize = vec3(
          (mainCanvas.width = correctedWidth),
          (mainCanvas.height = innerHeight)
        );
    }
    ...
    mainCanvas.style.width = glCanvas.style.width =
      innerAspect < fixedAspect ? "100%" : "";
    mainCanvas.style.height = glCanvas.style.height =
      innerAspect < fixedAspect ? "" : "100%";
  }
  ...
  const debugSpeedUp =
    devMode && (keyIsDown("Equal") || keyIsDown("NumpadAdd")); // +
  const debugSpeedDown =
    (devMode && keyIsDown("Minus")) || keyIsDown("NumpadSubtract"); // -
  if (debug)
    // +/- to speed/slow time
    frameTimeDeltaMS *= debugSpeedUp ? 20 : debugSpeedDown ? 0.1 : 1;
...
function enhancedModeUpdate() {
  ...
  if (document.hasFocus()) {
    if (autoFullscreen && !isFullscreen()) toggleFullscreen();
    autoFullscreen = 0;
  }
  ...
  if (keyWasPressed("KeyM")) {
    // toggle mute
    if (soundVolume) sound_bump.play(0.4, 3);
    soundVolume = soundVolume ? 0 : 0.3;
    if (soundVolume) sound_bump.play();
  }
  if (keyWasPressed("KeyR")) {
    // restart
    titleScreenMode = 0;
...
function updateCamera() {
  ...
  cameraPos.y =
    playerTrackInfo.offset.y + (titleScreenMode ? 1e3 : cameraPlayerOffset.y);

  // move camera with player
  cameraPos.x = playerVehicle.pos.x;

  // slight tilt camera with road
  cameraRot.x = lerp(0.1, cameraRot.x, cameraTrackInfo.pitch / 3);
