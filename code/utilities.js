...
const mod = (dividend, divisor = 1) =>
  ((dividend % divisor) + divisor) % divisor;
const clamp = (value, min = 0, max = 1) =>
  value < min ? min : value > max ? max : value;
const clampAngle = (value) =>
  ((((value + PI) % (2 * PI)) + 2 * PI) % (2 * PI)) - PI;
const percent = (value, valueA, valueB) =>
  (valueB -= valueA) ? clamp((value - valueA) / valueB) : 0;
const lerp = (percent, valueA, valueB) =>
  valueA + clamp(percent) * (valueB - valueA);
...
function formatTimeString(t) {
  ...
  return `${timeM}:${timeS < 10 ? "0" + timeS : timeS}.${
    (timeMS < 10 ? "00" : timeMS < 100 ? "0" : "") + timeMS
  }`;
}
...
class Vector3 {
  ...
  normalize(length = 1) {
    const l = this.length();
    return l ? this.scale(length / l) : vec3(length);
  }
  ...
  angleBetween(v) {
    ASSERT_VEC3(v);
    return Math.acos(clamp(this.dot(v), -1, 1));
  }
  clamp(a, b) {
    return vec3(clamp(this.x, a, b), clamp(this.y, a, b), clamp(this.z, a, b));
  }
  cross(v) {
    ASSERT_VEC3(v);
    return vec3(
  ...
  lerp(v, p) {
    ASSERT_VEC3(v);
    return v.subtract(this).scale(clamp(p)).addSelf(this);
  }
  rotateX(a) {
    const c = Math.cos(a),
      s = Math.sin(a);
  ...
  getHSLColor(a = 1) {
    return hsl(this.x, this.y, this.z, a);
  }
}
...
class Random {
  ...
  float(a = 1, b = 0) {
    ...
    if (js13kBuild)
      return (
        b + ((a - b) * Math.abs(this.seed % 1e9)) / 1e9
      ); // bias low values due to float error
    else return b + ((a - b) * Math.abs(this.seed % 1e8)) / 1e8;
  }
  floatSign(a, b) {
    return this.float(a, b) * this.sign();
  }
  int(a, b) {
    return this.float(a, b) | 0;
  }
  bool(chance = 0.5) {
  ...
  circle(radius = 0, bias = 0.5) {
    const r = this.float() ** bias * radius;
    const a = this.float(PI * 2);
    return vec3(r * Math.cos(a), r * Math.sin(a));
  }
  mutateColor(color, amount = 0.1, brightnessAmount = 0) {
    return rgb(
      clamp(
        random.float(1, 1 - brightnessAmount) *
  ...
  fromList(list, startBias = 1) {
    return list[(this.float() ** startBias * list.length) | 0];
  }
}
