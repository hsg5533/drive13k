...
function initSprites() {
  ...
  // trees
  spriteList.tree_palm = new GameSprite(vec3(0, 1), 1500, 0.2, 0.1, 0.04);
  spriteList.tree_palm.trackFace = 1;
  spriteList.tree_oak = new GameSprite(vec3(1, 1), 2e3, 0.5, 0.06, 0.1);
  spriteList.tree_stump = new GameSprite(vec3(2, 1), 1e3, 0.6, 0.04);
  spriteList.tree_dead = new GameSprite(vec3(3, 1), 800, 0.3, 0.03, 0.06);
  spriteList.tree_pink = new GameSprite(vec3(4, 1), 1500, 0.3, 0.1, 0.04);
  spriteList.tree_pink.trackFace = 1;
  spriteList.tree_bush = new GameSprite(vec3(5, 1), 1e3, 0.5, 0.1, 0.06);
  spriteList.tree_fall = new GameSprite(vec3(6, 1), 1500, 0.3, 0.1, 0.1);
  //TB(spriteList.tree_flower = new GameSprite(vec3(7,1),2e3,.3,.05,200));
  spriteList.tree_snow = new GameSprite(vec3(4, 3), 1300, 0.3, 0.06, 0.1);
  spriteList.tree_yellow = new GameSprite(vec3(5, 3), 1e3, 0.3, 0.06, 0.1);
  spriteList.tree_huge = new GameSprite(vec3(3, 1), 1e4, 0.5, 0.1, 0.1);
  spriteList.tree_huge.colorHSL = vec3(0.8, 0, 0.5);
  ...
  spriteList.tree_palm.shadowScale =
    ...
    spriteList.tree_yellow.shadowScale =
      0.7;

  // grass and flowers
  spriteList.grass_plain = new GameSprite(vec3(0, 3), 500, 0.5, 1);
  spriteList.grass_plain.colorHSL = vec3(0.3, 0.4, 0.5);
  spriteList.grass_dead = new GameSprite(vec3(0, 3), 600, 0.3, 1);
  spriteList.grass_dead.colorHSL = vec3(0.13, 0.6, 0.7);
  spriteList.grass_flower1 = new GameSprite(vec3(1, 3), 500, 0.3, 1);
  spriteList.grass_flower2 = new GameSprite(vec3(2, 3), 500, 0.3, 1);
  spriteList.grass_flower3 = new GameSprite(vec3(3, 3), 500, 0.3, 1);
  spriteList.grass_red = new GameSprite(vec3(0, 3), 700, 0.3, 1);
  spriteList.grass_red.colorHSL = vec3(0, 0.8, 0.5);
  spriteList.grass_snow = new GameSprite(vec3(0, 3), 300, 0.5, 1);
  spriteList.grass_snow.colorHSL = vec3(0.4, 1, 0.9);
  spriteList.grass_large = new GameSprite(vec3(0, 3), 1e3, 0.5, 1);
  spriteList.grass_large.colorHSL = vec3(0.4, 0.4, 0.5);
  ...
  PB(
    (spriteList.sign_opGames = new GameSprite(vec3(5, 2), 600, 0, 0.02, 0.5, 0))
  );
  PB((spriteList.sign_js13k = new GameSprite(vec3(0, 2), 600, 0, 0.02, 1, 0)));
  PB((spriteList.sign_zzfx = new GameSprite(vec3(1, 2), 500, 0, 0.02, 0.5, 0)));
  PB(
    (spriteList.sign_avalanche = new GameSprite(vec3(7, 2), 600, 0, 0.02, 1, 0))
  );
  PB(
    (spriteList.sign_github = new GameSprite(vec3(2, 2), 750, 0, 0.02, 0.5, 0))
  );
  PB((spriteList.sign_harris = new GameSprite(vec3(4, 2), 300, 0, 0.02, 1, 0)));
  spriteList.sign_frankForce = new GameSprite(vec3(3, 2), 500, 0, 0.02, 1, 0);
  //PB(spriteList.sign_dwitter   = new GameSprite(vec3(6,2),550,0,.02,1,0));

  // signs
  spriteList.sign_turn = new GameSprite(vec3(0, 5), 500, 0, 0.05, 0.5);
  ...
  // rocks
  spriteList.rock_tall = new GameSprite(vec3(1, 4), 1e3, 0.3, 0, 0.6, 0);
  spriteList.rock_big = new GameSprite(vec3(2, 4), 800, 0.2, 0, 0.57, 0);
  spriteList.rock_huge = new GameSprite(vec3(1, 4), 5e3, 0.7, 0, 0.6, 0);
  spriteList.rock_huge.shadowScale = 0;
  spriteList.rock_huge.colorHSL = vec3(0.08, 1, 0.8);
  spriteList.rock_huge.hueRandomness = 0.01;
  spriteList.rock_huge2 = new GameSprite(vec3(2, 4), 8e3, 0.5, 0, 0.25, 0);
  spriteList.rock_huge2.shadowScale = 0;
  spriteList.rock_huge2.colorHSL = vec3(0.05, 1, 0.8);
  spriteList.rock_huge2.hueRandomness = 0.01;
  spriteList.rock_huge3 = new GameSprite(vec3(2, 4), 8e3, 0.7, 0, 0.5, 0);
  spriteList.rock_huge3.shadowScale = 0;
  spriteList.rock_huge3.colorHSL = vec3(0.05, 1, 0.8);
  spriteList.rock_huge3.hueRandomness = 0.01;
  spriteList.rock_weird = new GameSprite(vec3(2, 4), 5e3, 0.5, 0, 1, 0);
  spriteList.rock_weird.shadowScale = 0;
  spriteList.rock_weird.colorHSL = vec3(0.8, 1, 0.8);
  spriteList.rock_weird.hueRandomness = 0.2;
  spriteList.rock_weird2 = new GameSprite(vec3(1, 4), 1e3, 0.5, 0, 0.5, 0);
  spriteList.rock_weird2.colorHSL = vec3(0, 0, 0.2);
  spriteList.tunnel1 = new GameSprite(vec3(6, 4), 1e4, 0.0, 0, 0, 0);
  spriteList.tunnel1.shadowScale = 0;
  spriteList.tunnel1.colorHSL = vec3(0.05, 1, 0.8);
  spriteList.tunnel1.tunnelArch = 1;
  spriteList.tunnel2 = new GameSprite(vec3(7, 4), 5e3, 0, 0, 0, 0);
  spriteList.tunnel2.shadowScale = 0;
  spriteList.tunnel2.colorHSL = vec3(0, 0, 0.1);
  ...
  spriteList.tunnel2Front.colorHSL = vec3(0, 0, 0.8);
  //spriteList.tunnel2_rock         = new GameSprite(vec3(6,6),1e4,.2,0,.5,0);
  //spriteList.tunnel2_rock.colorHSL = vec3(.15, .5, .8);

  // hazards
  spriteList.hazard_rocks = new GameSprite(vec3(3, 4), 600, 0.2, 0, 0.9);
  spriteList.hazard_rocks.shadowScale = 0;
  spriteList.hazard_rocks.isBump = 1;
  spriteList.hazard_rocks.spriteYOffset = -0.02;
  spriteList.hazard_sand = new GameSprite(vec3(4, 4), 600, 0.2, 0, 0.9);
  spriteList.hazard_sand.shadowScale = 0;
  spriteList.hazard_sand.isSlow = 1;
  spriteList.hazard_sand.spriteYOffset = -0.02;
  //spriteList.hazard_snow     = new GameSprite(vec3(6,6),500,.1,0,300,0);
  //spriteList.hazard_snow.isSlow = 1;

  // special sprites
  spriteList.water = new GameSprite(vec3(5, 4), 6e3, 0.5, 1);
  spriteList.water.shadowScale = 0;
  spriteList.sign_start = new GameSprite(vec3(1, 6), 2300, 0, 0.01, 0, 0);
  spriteList.sign_start.shadowScale = 0;
  spriteList.sign_goal = new GameSprite(vec3(0, 6), 2300, 0, 0.01, 0, 0);
  spriteList.sign_goal.shadowScale = 0;
  spriteList.sign_checkpoint1 = new GameSprite(vec3(6, 0), 1e3, 0, 0.01, 0, 0);
  spriteList.sign_checkpoint1.shadowScale = 0;
  spriteList.sign_checkpoint2 = new GameSprite(vec3(7, 0), 1e3, 0, 0.01, 0, 0);
  spriteList.sign_checkpoint2.shadowScale = 0;
  spriteList.telephonePole = new GameSprite(vec3(0, 4), 1800, 0, 0, 0.03, 0);
  //spriteList.parts_girder  = new GameSprite(vec3(0,6),500,0,.05,30,0);
  spriteList.telephonePole.shadowScale = 0.3;
  spriteList.grave_stone = new GameSprite(vec3(2, 6), 500, 0.3, 0.05, 0.5, 0);
  spriteList.grave_stone.lightnessRandomness = 0.5;
...
class GameSprite {
  constructor(
    ...
    this.hueRandomness = 0.05;
    this.lightnessRandomness = 0.01;
  }
  ...
  getRandomSpriteScale() {
    return 1 + random.floatSign(this.sizeRandomness);
  }

  randomize() {
    this.colorHSL.x = random.float(-0.1, 0.1);
    this.colorHSL.y = clamp(this.colorHSL.y + random.float(-0.1, 0.1));
    this.colorHSL.z = clamp(this.colorHSL.z + random.float(-0.1, 0.1));
    this.hueRandomness = 0.05;
    this.lightnessRandomness = 0.01;
  }
}
...
function drawInit() {
  ...
  }
  {
    ...
    const uvs1 = points1.map((p) =>
      p.multiply(vec3(0.5, -0.5, 0.5)).add(vec3(0.5))
    );
    quadMesh = new Mesh(
      points1,
      points1.map((p) => vec3(0, 0, 1)),
      uvs1
    );
  ...
  }
  {
    ...
    carMesh = new Mesh().buildExtrude(points, 0.5);
...
class Mesh {
  ...
  renderTile(transform, color, tile) {
    ...
    const uvs = this.uvs.map((uv) =>
      vec3(spriteSize - spriteSize * uv.x + tile.x, uv.y * spriteSize + tile.y)
    );
  ...
  buildExtrude(facePoints, size = 1) {
    ...
    for (let k = 2; k--; )
      for (let i = vertCount; i--; ) {
        ...
        if (!k)
          // hack to fix glitch in mesh due to concave shape
...
function pushSprite(pos, size, color, tile, skew = 0) {
  ...
  if (tile) {
    ...
    if (size.x < 0) tilePosX -= tileSizeX *= -1;
    if (size.y < 0) tilePosY -= tileSizeY *= -1;
    const uvs = mesh.uvs.map((uv) =>
      vec3(uv.x * tileSizeX + tilePosX, uv.y * tileSizeY + tilePosY)
    );
    glPushVertsCapped(points, 0, color, uvs);
  } else glPushVertsCapped(points, 0, color);
}

function pushShadow(pos, xSize, zSize) {
  if (optimizedCulling && pos.z > 2e4) return; // cull far shadows

  const color = rgb(0, 0, 0, 0.7);
  const tile = spriteList.dot.spriteTile;
  const mesh = shadowMesh;
  const points = mesh.points.map((p) =>
    vec3(p.x * xSize + pos.x, pos.y, p.z * zSize + pos.z)
  );
  const uvs = mesh.uvs.map((uv) =>
