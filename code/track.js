...
function drawRoad(zwrite) {
  ...
  for (let i = drawDistance, segment1, segment2; i--; ) {
    ...
    if (i % (lerp(i / drawDistance, 1, 8) | 0))
...
function drawTrackScenery() {
  ...
  for (let i = drawDistance; i--; ) {
    ...
    for (const trackObject of trackSegment.trackObjects) trackObject.draw();

    // random scenery
    const levelInfo = getLevelInfo(trackSegment.level);
    const w = trackSegment.width;
    if (!trackSegment.sideStreet)
      // no sprites on side streets
      for (let k = 3; k--; ) {
        ...
        if (spriteSide == levelInfo.waterSide) {
          ...
          const levelLerpPercent = percent(
            levelFloat % 1,
            1 - levelLerpRange,
            1
          );
          const sceneryLevelInfo = random.bool(levelLerpPercent)
            ? levelInfoNext
            : levelInfo;
          ...
          if (sceneryLevelInfo.scenery) {
            ...
            const scale = vec3(s);
            if (sprite.canMirror && random.bool()) scale.x *= -1;
  ...
  if (!js13kBuild) {
    // final thing rendered, so no need to reset
    glSetDepthTest();
    glEnableLighting = 1;
  }
}
...
function pushTrackObject(pos, scale, color, sprite, trackWind) {
  if (optimizedCulling) {
    const cullScale = 200;
    if (cullScale * scale.y < pos.z) return; // cull out small sprites
    if (abs(pos.x) - abs(scale.x) > pos.z * 4 + 2e3) return; // out of view
    if (pos.z < 0) return; // behind camera
  }
  ...
  const spriteYOffset =
    scale.y * (1 + sprite.spriteYOffset) + (freeCamMode ? cameraPos.y / 20 : 0);
...
class TrackObject {
  constructor(trackSegment, sprite, offset, color = WHITE, sizeScale = 1) {
    ...
    if (trackside && sprite.trackFace) this.scale.x *= sign(offset.x);
    else if (sprite.canMirror && random.bool()) this.scale.x *= -1;
...
class TrackSegment {
  constructor(segmentIndex, offset, width) {
    if (segmentIndex >= levelGoal * checkpointTrackSegments) width = 0; // no track after end
    ...
    const level = (this.level = testLevelInfo
      ? testLevelInfo.level
      : levelFloat | 0);
    ...
    const checkpointLine =
      (segmentIndex > 25 && segmentIndex < 30) ||
    ...
    this.sideStreet =
      levelInfo.sideStreets &&
      (segmentIndex % checkpointTrackSegments) % 495 < 36;

    {
      // setup colors
      const groundColor = levelInfo.groundColor.lerp(
        levelInfoNext.groundColor,
        levelLerpPercent
      );
      const lineColor = levelInfo.lineColor.lerp(
        levelInfoNext.lineColor,
        levelLerpPercent
      );
      const roadColor = levelInfo.roadColor.lerp(
        levelInfoNext.roadColor,
        levelLerpPercent
      );

      const largeSegmentIndex = (segmentIndex / 9) | 0;
      const stripe = largeSegmentIndex % 2 ? 0.1 : 0;
      this.colorGround = groundColor.brighten(
        Math.cos((segmentIndex * 2) / PI) / 20
      );
      this.colorRoad = roadColor.brighten(stripe && 0.05);
      if (recordPointLine) this.colorRoad = hsl(0, 0.8, 0.5);
      else if (checkpointLine) this.colorRoad = WHITE; // starting line
      this.colorLine = lineColor;
      if (stripe) this.colorLine.a = 0;
      if (this.sideStreet) this.colorLine = this.colorGround = this.colorRoad;
    }
    ...
    if (segmentIndex == 30) {
      ...
      if (js13kBuild) random.seed = 1055752394; // hack, reset seed for js13k
    }
  }
...
class TrackSegmentInfo {
  constructor(z) {
    ...
    if (track[segment] && track[segment + 1]) {
      ...
      else this.pos = vec3(0, 0, z);
      this.pitch = lerp(
        percent,
        track[segment].pitch,
        track[segment + 1].pitch
      );
      this.offset = track[segment].offset.lerp(
        track[segment + 1].offset,
        percent
      );
      this.width = lerp(
        percent,
        track[segment].width,
        track[segment + 1].width
      );
    } else this.offset = this.pos = vec3((this.pitch = this.width = 0), 0, z);
  }
}
