...
function initLevelInfos() {
  ...
  LI = new LevelInfo(
    level++,
    [spriteList.grass_plain, spriteList.tree_palm, spriteList.rock_big],
    spriteList.tree_palm
  );
  LI.waterSide = -1;
  LI.horizonSpriteSize = 0.5;
  //LI.tunnel = spriteList.tunnel2; // test tunnel
  LI.billboardChance = 0.3; // more billboards at start
  ...
  LI = new LevelInfo(
    ...
    spriteList.tree_bush,
    spriteList.horizon_smallMountains
  );
  LI.trackSideRate = 10;
  LI.sceneryListBias = 9;
  //LI.skyColorTop = WHITE;
  LI.skyColorBottom = hsl(0.5, 0.3, 0.5);
  LI.roadColor = hsl(0.05, 0.4, 0.2);
  LI.groundColor = hsl(0.2, 0.4, 0.4);
  LI.cloudColor = hsl(0, 0, 1, 0.3);
  LI.cloudHeight = 0.2;
  LI.sunHeight = 0.7;
  LI.billboardChance = 0.1; // less billboards in forest type areas
  //LI.trafficDensity = .7; // less traffic in forest

  // trail through forest
  LI.turnChance = 0.7; // more small turns
  //LI.turnMin = 0;
  //LI.turnMax = .6;
  LI.bumpChance = 0.8;
  LI.bumpFreqMin = 0.4;
  ...
  LI = new LevelInfo(
    ...
    spriteList.telephonePole,
    spriteList.horizon_desert
  );
  LI.trackSideRate = 50;
  LI.trackSideChance = 1;
  LI.skyColorTop = hsl(0.15, 1, 0.9);
  LI.skyColorBottom = hsl(0.5, 0.7, 0.6);
  LI.roadColor = hsl(0.1, 0.2, 0.2);
  LI.lineColor = hsl(0, 0, 1, 0.5);
  LI.groundColor = hsl(0.1, 0.2, 0.5);
  ...
  LI.hazardChance = 0.005;
  LI.tunnel = spriteList.tunnel2;
  LI.trafficDensity = 0.7; // less traffic in desert, only 2 lanes
  LI.billboardRate = 87;
  LI.billboardScale = 8;
  ...
  LI = new LevelInfo(
    ...
    spriteList.tree_snow,
    spriteList.horizon_snow
  );
  LI.trackSideRate = 31;
  LI.skyColorTop = hsl(0.5, 0.2, 0.4);
  LI.skyColorBottom = WHITE;
  LI.roadColor = hsl(0, 0, 0.5, 0.3);
  LI.groundColor = hsl(0.6, 0.3, 0.9);
  LI.cloudColor = hsl(0, 0, 0.8, 0.5);
  LI.horizonSpriteSize = 3;
  LI.lineColor = hsl(0, 0, 1, 0.5);
  LI.sunHeight = 0.7;
  LI.hazardType = spriteList.hazard_rocks;
  LI.hazardChance = 0.002;
  ...
  LI = new LevelInfo(
    ...
    spriteList.tree_fall,
    spriteList.horizon_brownMountains
  );
  LI.sceneryListBias = 2;
  LI.trackSideRate = 31;
  LI.skyColorTop = hsl(0.7, 1, 0.7);
  LI.skyColorBottom = hsl(0.2, 1, 0.9);
  LI.roadColor = hsl(0, 0, 0.15);
  LI.groundColor = hsl(0.1, 0.4, 0.5);
  LI.cloudColor = hsl(0, 0, 1, 0.3);
  LI.cloudHeight = 0.1;
  LI.sunColor = hsl(0, 1, 0.7);
  //LI.laneCount = 3;
  LI.billboardChance = 0.1; // less billboards in forest type areas
  LI.trafficDensity = 0.7; // less traffic in canyon
  ...
  LI = new LevelInfo(
    ...
    spriteList.tree_yellow,
    spriteList.horizon_city
  );
  LI.trackSideRate = 31;
  LI.skyColorTop = YELLOW;
  LI.skyColorBottom = RED;
  LI.roadColor = hsl(0, 0, 0.1);
  LI.lineColor = hsl(0.15, 1, 0.7);
  LI.groundColor = hsl(0.05, 0.5, 0.4);
  LI.cloudColor = hsl(0.15, 1, 0.5, 0.5);
  //LI.cloudHeight = .3;
  LI.billboardRate = 23; // more billboards in city
  LI.billboardChance = 0.5;
  LI.horizonSpriteSize = 1;
  if (!js13kBuildLevel2) LI.horizonFlipChance = 0.3;
  LI.sunHeight = 0.5;
  LI.sunColor = hsl(0.15, 1, 0.8);
  LI.sideStreets = 1;
  LI.laneCount = 5;
  ...
  LI.turnChance = 0.3;
  LI.turnMin = 0.5;
  LI.turnMax = 0.9; // bigger turns since lanes are wide
  //LI.bumpChance = .5;
  LI.bumpFreqMin = 0.3;
  LI.bumpFreqMax = 0.6;
  LI.bumpScaleMin = 80;
  ...
  LI = new LevelInfo(
    ...
    spriteList.tree_oak,
    spriteList.horizon_graveyard
  );
  LI.sceneryListBias = 2;
  LI.trackSideRate = 50;
  LI.skyColorTop = hsl(0.5, 1, 0.5);
  LI.skyColorBottom = hsl(0, 1, 0.8);
  LI.roadColor = hsl(0.6, 0.3, 0.15);
  LI.groundColor = hsl(0.2, 0.3, 0.5);
  LI.lineColor = hsl(0, 0, 1, 0.5);
  LI.billboardChance = 0; // no ads in graveyard
  LI.cloudColor = hsl(0.15, 1, 0.9, 0.3);
  ...
  LI.turnMax = 0.6;
  LI.bumpChance = 0.6;
  LI.bumpFreqMin = LI.bumpFreqMax = 0.7;
  LI.bumpScaleMin = 80;
  //LI.bumpScaleMax = 150;
  ...
  LI = new LevelInfo(
    ...
    spriteList.rock_big,
    spriteList.horizon_redMountains
  );
  LI.sceneryListBias = 5;
  LI.trackSideRate = 25;
  LI.skyColorTop = hsl(0, 1, 0.8);
  LI.skyColorBottom = hsl(0.6, 1, 0.6);
  LI.lineColor = hsl(0, 0, 0, 0);
  LI.roadColor = hsl(0, 0.6, 0.2, 0.8);
  LI.groundColor = hsl(0.1, 0.5, 0.4);
  LI.waterSide = 1;
  LI.cloudColor = hsl(0, 1, 0.96, 0.8);
  LI.cloudWidth = 0.6;
  //LI.cloudHeight = .3;
  LI.sunHeight = 0.7;
  LI.sunColor = hsl(0.1, 1, 0.7);
  ...
  LI.turnChance = 0.8;
  //LI.turnMin = 0;
  LI.turnMax = 0.3; // lots of slight turns
  LI.bumpChance = 1;
  LI.bumpFreqMin = 0.4;
  LI.bumpFreqMax = 0.6;
  LI.bumpScaleMin = 10;
  ...
  LI = new LevelInfo(
    level++,
    [spriteList.grass_red, spriteList.rock_weird, spriteList.tree_huge],
    spriteList.rock_weird2,
    spriteList.horizon_weird
  );
  LI.trackSideRate = 50;
  LI.skyColorTop = hsl(0.05, 1, 0.8);
  LI.skyColorBottom = hsl(0.15, 1, 0.7);
  LI.lineColor = hsl(0, 1, 0.9);
  LI.roadColor = hsl(0.6, 1, 0.1);
  LI.groundColor = hsl(0.6, 1, 0.6);
  LI.cloudColor = hsl(0.9, 1, 0.5, 0.3);
  LI.cloudHeight = 0.2;
  LI.sunColor = BLACK;
  LI.laneCount = 4;
  ...
  LI.trackSideRate = 21;
  LI.skyColorTop = hsl(0.2, 1, 0.9);
  LI.skyColorBottom = hsl(0.55, 1, 0.5);
  LI.roadColor = hsl(0, 0, 0.1);
  LI.groundColor = hsl(0.1, 0.5, 0.7);
  LI.cloudColor = hsl(0, 0, 1, 0.5);
  LI.tunnel = spriteList.tunnel1;
  if (js13kBuildLevel2) LI.horizonSpriteSize = 0;
  else {
    LI.sunHeight = 0.6;
...
const getLevelInfo = (level) =>
  testLevelInfo || levelInfoList[level | 0] || levelInfoList[0];
...
class LevelInfo {
  constructor(
    ...
    scenery,
    trackSideSprite,
    horizonSprite = spriteList.horizon_islands
  ) {
    // add self to list
    levelInfoList[level] = this;

    if (debug) {
      for (const s of scenery) ASSERT(s, "missing scenery!");
    }
    ...
    this.billboardChance = 0.2;
    this.billboardRate = 45;
    this.billboardScale = 1;
    this.trackSideRate = 5;
    this.trackSideForce = 0;
    this.trackSideChance = 0.5;

    this.groundColor = hsl(0.08, 0.2, 0.7);
    this.skyColorTop = WHITE;
    this.skyColorBottom = hsl(0.57, 1, 0.5);
    this.lineColor = WHITE;
    this.roadColor = hsl(0, 0, 0.5);

    // horizon stuff
    this.cloudColor = hsl(0.15, 1, 0.95, 0.7);
    this.cloudWidth = 1;
    this.cloudHeight = 0.3;
    this.horizonSprite = horizonSprite;
    this.horizonSpriteSize = 2;
    this.sunHeight = 0.8;
    this.sunColor = hsl(0.15, 1, 0.95);
    ...
    this.bumpFreqMax = 0.7; // more often bumps
  ...
  randomize() {
    ...
    this.trafficDensity = random.float(0.5, 1.5);
    ...
    this.bumpFreqMin = random.float(0.5); // no bumps
