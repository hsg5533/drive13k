...
function drawSky() {
  ...
  {
    // top/bottom gradient
    const skyColorTop = levelInfoLast.skyColorTop.lerp(
      levelInfo.skyColorTop,
      levelLerpPercent
    );
    const skyColorBottom = levelInfoLast.skyColorBottom.lerp(
      levelInfo.skyColorBottom,
      levelLerpPercent
    );
    pushGradient(
      vec3(0, skyH, skyZ).addSelf(cameraPos),
      vec3(skyW, skyH),
      skyColorTop,
      skyColorBottom
    );
    ...
    glLightColor = skyColorTop.lerp(WHITE, 0.8).lerp(BLACK, 0.3);
    glAmbientColor = skyColorBottom.lerp(WHITE, 0.8).lerp(BLACK, 0.3);
    glFogColor = skyColorBottom.lerp(WHITE, 0.5);
  }
  ...
  const dotSpriteTile = spriteList.dot.spriteTile;
  {
    ...
    const sunHeight =
      skyTop *
      lerp(levelLerpPercent, levelInfoLast.sunHeight, levelInfo.sunHeight);
    const sunColor = levelInfoLast.sunColor.lerp(
      levelInfo.sunColor,
      levelLerpPercent
    );
    ...
    for (let i = 0; i < 1; i += 0.05) {
      ...
      pushSprite(
        vec3(x * headingScale, sunHeight, skyZ).addSelf(cameraPos),
        vec3(sunSize * (1 + i * 30)),
        sunColor,
        i ? dotSpriteTile : circleSpriteTile
      );
    }
  }
  ...
  const windSpeed = 50;
  for (let i = 99; i--; ) {
    const cloudColor = levelInfoLast.cloudColor.lerp(
      levelInfo.cloudColor,
      levelLerpPercent
    );
    const cloudWidth = lerp(
      levelLerpPercent,
      levelInfoLast.cloudWidth,
      levelInfo.cloudWidth
    );
    const cloudHeight = lerp(
      levelLerpPercent,
      levelInfoLast.cloudHeight,
      levelInfo.cloudHeight
    );

    let x =
      worldHeading * headingScale +
      random.float(range) +
      time * windSpeed * random.float(1, 1.5);
    x = mod(x, range) - range / 2;
    const y = random.float(skyTop);
    const s = random.float(3e2, 8e2);
    pushSprite(
      vec3(x, y, skyZ).addSelf(cameraPos),
      vec3(s * cloudWidth, s * cloudHeight),
      cloudColor,
      dotSpriteTile
    );
  }
  ...
  for (let i = 99; i--; ) {
    ...
    const levelTransition =
      levelFloat < 0.5 || levelFloat > levelGoal - 0.5
        ? 1
        : levelPercent < ltt
        ? (levelPercent / ltt) ** ltp
        : levelPercent > 1 - ltt
        ? 1 - ((levelPercent - 1) / ltt + 1) ** ltp
        : 1;

    const parallax = lerp(p, 1.01, 1.1);
    const s = random.float(1e2, 2e2) * horizonSpriteSize;
    const size = vec3(
      random.float(1, 2) * (horizonSprite.canMirror ? s * random.sign() : s),
      s,
      s
    );
    const x =
      mod(
        (worldHeading * headingScale) / parallax + random.float(range),
        range
      ) -
      range / 2;

    const yMax = size.y * 0.75;
    if (!js13kBuildLevel2 && levelInfo.horizonFlipChance) {
      // horizon spites that can be flipped vertically
      if (random.bool(levelInfo.horizonFlipChance)) size.y *= -1;
    }
...

  {
    ...
    // horizon bottom
    const groundColor = levelInfoLast.groundColor
      .lerp(levelInfo.groundColor, levelLerpPercent)
      .brighten(0.1);
    pushSprite(
      vec3(0, -skyH, skyZ).addSelf(cameraPos),
      vec3(skyW, skyH),
