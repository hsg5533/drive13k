...
function debugUpdate() {
  ...
  if (freeCamMode) {
    ...
    freeCamRot = freeCamRot.add(
      vec3(mouseDelta.y, mouseDelta.x).scale(turnSpeed)
    );
  ...
  if (keyWasPressed("Digit5")) checkpointTimeLeft = 12;
  if (keyWasPressed("Digit6")) {
    ...
    for (const s in spriteList) {
      ...
      if (sprite instanceof GameSprite) sprite.randomize();
    }
  ...
  if (keyWasPressed("Digit7")) debugGenerativeCanvas = !debugGenerativeCanvas;
  if (keyWasPressed("Digit0")) debugCapture = 1;
  if (keyWasPressed("KeyQ") && !freeCamMode) testDrive = !testDrive;
  if (keyWasPressed("KeyU")) sound_win.play();
  if (debug && keyWasPressed("KeyV")) spawnVehicle(playerVehicle.pos.z - 1300);
...
function debugDraw() {
  ...
  if (testDrive && !titleScreenMode && !freeRide)
    drawHUDText("AUTO", vec3(0.5, 0.95), 0.05, RED);
  ...
  if (showMap) {
    ...
    for (let k = 2; k--; ) {
      ...
      for (let i = 0; i < 1e3; i++) {
        ...
        const o = t.offset;
        v += o.x;
        p = p.add(d.rotateZ(v * 0.005));
        if (j % 5 == 0) {
          let y = o.y;
          let w = t.width / 199;
          const h = k ? 5 : -y * 0.01;
          context.fillStyle = hsl(y * 0.0001, 1, k ? 0 : 0.5, k ? 0.5 : 1);
  ...
  if (0) {
    ...
    cubeMesh.render(m1, hsl(0, 0.8, 0.5));
  }
