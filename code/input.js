...
const gamepadWasPressed = (key, gamepad = 0) =>
  !!(gamepadData[gamepad][key] & 2);
const gamepadWasReleased = (key, gamepad = 0) =>
  !!(gamepadData[gamepad][key] & 4);
const gamepadStick = (stick, gamepad = 0) =>
  gamepadStickData[gamepad]
    ? gamepadStickData[gamepad][stick] || vec3()
    : vec3();
...
function inputInit() {
  ...
  onkeydown = (e) => {
    ...
    if (!e.repeat) {
      inputData[e.code] = 3;
      if (inputWASDEmulateDirection) inputData[remapKey(e.code)] = 3;
    }
  };

  onkeyup = (e) => {
    inputData[e.code] = 4;
    if (inputWASDEmulateDirection) inputData[remapKey(e.code)] = 4;
  };
...
function inputUpdatePost() {
  ...
  for (const i in inputData) inputData[i] &= 1;
}
...
function gamepadsUpdate() {
  ...
  if (!devMode && !document.hasFocus()) return;
    ...
    if (gamepad) {
      ...
      for (let j = 0; j < gamepad.axes.length - 1; j += 2)
        sticks[j >> 1] = applyDeadZones(
          vec3(gamepad.axes[j], gamepad.axes[j + 1])
        );
      ...
      for (let j = gamepad.buttons.length; j--; ) {
        ...
        dataValue[j] = percent(button.value || 0, 0.1, 0.9); // apply deadzone
...
function touchInputInit() {
  ...
  function handleTouchDefault(e) {
    ...
    if (soundEnable && !audioContext) audioContext = new AudioContext(); // create audio context
    ...
    } else if (wasTouching) inputData[button] = (inputData[button] & 2) | 4;
...
function handleTouchGamepad(e) {
  if (soundEnable) {
    if (!audioContext) audioContext = new AudioContext(); // create audio context
