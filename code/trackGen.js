...
function buildTrack() {
  ...
  const roadTransitionRange = testQuick
    ? min(checkpointTrackSegments, 500)
    : 500;
  for (let i = 0; i < trackEnd + 5e4; ++i) {
    ...
    const levelLerpPercent = percent(
      i % checkpointTrackSegments,
      0,
      roadTransitionRange
    );

    if (js13kBuild && i == 31496) random.setSeed(7); // mess with seed to randomize jungle

    const roadGenWidth =
      (laneWidth / 2) *
      lerp(levelLerpPercent, levelInfoLast.laneCount, levelInfo.laneCount);
    ...
    let isBetweenLevels =
      checkpointSegment < levelBetweenRange ||
    ...
    if (isBetweenLevels) {
      ...
      if (sectionXEndDistance-- < 0) {
        ...
        sectionXEndDistance = random.int(
          sectionDistanceMin,
          sectionDistanceMax
        );
        sectionTurn = random.bool(turnChance)
          ? random.floatSign(turnMin, turnMax)
          : 0;
      }
      ...
      if (sectionYEndDistance-- < 0) {
        // pick random section distance
        sectionYEndDistance = random.int(
          sectionDistanceMin,
          sectionDistanceMax
        );
    ...
    if (i > trackEnd - 500) sectionTurn = 0; // no turns at end

    turn = lerp(0.02, turn, sectionTurn); // smooth out turns

    // apply noise to height
    const noiseFrequency = (currentNoiseFrequency = lerp(
      0.01,
      currentNoiseFrequency,
      sectionBumpFrequency
    ));
    const noiseSize = (currentNoiseScale = lerp(
      0.01,
      currentNoiseScale,
      sectionBumpScale
    ));
    ...
    if (currentNoiseFrequency) noisePos += noiseFrequency / noiseSize;
  ...
  for (let i = 0; i < track.length; ++i) {
    ...
    if (t.sideStreet || i < 50) continue; // no objects on side streets
    ...
    let signSide;
    if (i < levelGoal * checkpointTrackSegments)
    ...
    if (levelInfo.tunnel) {
      ...
      if (iCheckpoint > 100 && iCheckpoint < checkpointTrackSegments - 100) {
        ...
        if (tunnelOn) {
          ...
          const sprite =
            isLongTunnel && !wasOn ? spriteList.tunnel2Front : levelInfo.tunnel;
...

    {
      ...
      if (i % billboardRate == 0 && random.bool(billboardChance)) {
        ...
        t.addSprite(
          billboardSprite,
          billboardSide * random.float(width + 600, width + 800),
          0,
          extraScale
        );
        continue;
      ...
      if (levelInfo.trackSideSprite) {
        ...
        if (random.bool(0.001)) {
          trackSideChanceScale = random.bool(0.4)
            ? 1 // normal to spawn often
            : random.bool(0.1)
            ? 0 // small chance of none
        ...
        if (
          i % trackSideRate == 0 &&
          random.bool(trackSideChance * trackSideChanceScale)
        ) {
          const trackSideForce = levelInfo.trackSideForce;
          const side =
            trackSideForce ||
            (i % (trackSideRate * 2) < trackSideRate ? 1 : -1);
          t.addSprite(
            levelInfo.trackSideSprite,
            side * (t.width + random.float(700, 1e3))
          );
          continue;
        }
      }
      ...
      if (iCheckpoint > 40 && iCheckpoint < checkpointTrackSegments - 40)
        if (
          hazardWait-- < 0 &&
          levelInfo.hazardType &&
          random.bool(levelInfo.hazardChance)
        ) {
          // hazards on the ground in road to slow player
          const sprite = levelInfo.hazardType;
          t.addSprite(sprite, random.floatSign(t.width / 0.9));
